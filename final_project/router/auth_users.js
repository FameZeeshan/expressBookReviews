...
regd_users.put("/auth/review/:isbn", (req, res) => {
  ...
  return res.status(404).json({
    message: `Error updating review for ${user} to requested isbn ${reqIsbn}`,
  });
});
...
regd_users.delete("/auth/review/:isbn", (req, res) => {
    ...
    for (const [key, value] of Object.entries(books)) {
      ...
      if (value["isbn"] == reqIsbn && userReview) {
        ...
        return res
          .status(200)
          .send(
            `Review deleted successfully for user - ${user} to isbn ${reqIsbn}`
          );
      }
    }
  }
  return res
    .status(404)
    .json({ message: `Error deleting review for ${user} to isbn ${reqIsbn}` });
});
